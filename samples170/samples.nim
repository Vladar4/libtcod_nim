...
proc renderNoise*(first: bool; key: ptr Key; mouse: ptr Mouse) {.cdecl.} =
  ...
  for curfunc in PERLIN.cint..TURBULENCE_WAVELET:
    if curfunc == funct:
      ...
      consolePrint(sampleConsole, 2, 2 + curfunc, funcName[curfunc])

  # draw parameters
  consoleSetDefaultForeground(sampleConsole, WHITE)
  consolePrint(sampleConsole, 2, 11, "Y/H : zoom (%2.1f)", zoom)
  if funct > WAVELET:
    consolePrint(sampleConsole, 2, 12, "E/D : hurst (%2.1f)", hurst)
    consolePrint(sampleConsole, 2, 13, "R/F : lacunarity (%2.1f)", lacunarity)
    consolePrint(sampleConsole, 2, 14, "T/G : octaves (%2.1f)", octaves)
...
proc renderMouse*(first: bool; key: ptr Key; mouse: ptr Mouse) {.cdecl.} =
  ...
  consolePrint(sampleConsole, 1, 10, "1 : Hide cursor\n2 : Show cursor")
  if key.vk == K_Text and key.text[0] != '\x00':
    if key.text[0] == '1':
...
proc renderPath*(first: bool; key: ptr Key; mouse: ptr Mouse) {.cdecl.} =
  ...
  if first:
    ...
    consolePrint(sampleConsole, 1, 4, "Using : A*")
    # draw windows
    for y in 0.cint..<SAMPLE_SCREEN_HEIGHT:
      for x in 0.cint..<SAMPLE_SCREEN_WIDTH:
        if smap[y][x] == '=':
  ...
  if key.vk == K_Tab:
    ...
    if usingAstar:
      ...
      consolePrint(sampleConsole, 1, 4, "Using : Dijkstra")
    recalculatePath = true
...
proc renderBsp*(first: bool; key: ptr Key; mouse: ptr Mouse) {.cdecl.} =
  ...
  if randomRoom:
    consolePrint(sampleConsole, 1, 6, "2 : room walls %s",
...
proc renderName*(first: bool; key: ptr Key; mouse: ptr Mouse) {.cdecl.} =
  ...
  for i in 0..names.high:
    ...
    if name.len < SAMPLE_SCREEN_WIDTH:
      consolePrintEx(sampleConsole, SAMPLE_SCREEN_WIDTH - 2, cint 2 + i, BKGND_NONE, RIGHT, name)
    ...
    for x in samplex..<(samplex + samplew):
      ...
      for y in sampley..<(sampley + sampleh):
        ...
        if (y - sampley) mod 8 == 0:
          ...
          n = noiseGetFbm(noise, floatArrayToPtr(f), 3.0)
        dc = cint(3 * (n + 1.0))
        count = 0
...
while true:
  ...
  for i in 0..<NB_RENDERERS.ord:
    if i == curRenderer.ord:
      ...
      consoleSetDefaultForeground(nil, GREY)
      consoleSetDefaultBackground(nil, BLACK)
    consolePrintEx(nil, 42, cint 46 - (NB_RENDERERS.ord - i), BKGND_SET, LEFT, rendererName[i])
